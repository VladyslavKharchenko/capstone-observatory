...
object Interaction2 extends Interaction2Interface {
  ...
  val temperaturesColors: Seq[(Temperature, Color)] = Seq(
  ...
  val deviationsColors: Seq[(Temperature, Color)] = Seq(
    (7, Color(0, 0, 0)),
    (4, Color(255, 0, 0)),
    (2, Color(255, 255, 0)),
    (0, Color(255, 255, 255)),
  ...
  def availableLayers: Seq[Layer] = {
    Seq(
      Layer(LayerName.Temperatures, temperaturesColors, 1975 to 2015),
      Layer(LayerName.Deviations, deviationsColors, 1975 to 2015))
  }
  ...
  def yearSelection(selectedLayer: Signal[Layer], sliderValue: Signal[Year]): Signal[Year] = {
    val selectedYear = sliderValue().min(selectedLayer().bounds.end).max(selectedLayer().bounds.start)
    Signal(selectedYear)
  }
